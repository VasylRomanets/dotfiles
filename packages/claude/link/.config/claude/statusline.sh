...
# Fields: context usage %, 5h/7d rate-limit usage, session cost, session duration,
# lines added/removed, model, effort level.
...
if [ -n "$five_hour_pct" ] || [ -n "$seven_day_pct" ]; then
    printf "${dim} • "
    [ -n "$five_hour_pct" ]                            && printf "5h:%.0f%%" "$five_hour_pct"
    [ -n "$five_hour_pct" ] && [ -n "$seven_day_pct" ] && printf " "
    [ -n "$seven_day_pct" ]                            && printf "7d:%.0f%%" "$seven_day_pct"
    printf "${reset}"
fi
